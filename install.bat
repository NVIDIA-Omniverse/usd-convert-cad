...
echo  Checking omniverse-kit package...
"%VENV_PY%" -m pip show omniverse-kit >nul 2>&1
if errorlevel 1 (
    ...
    echo  [OK] omniverse-kit package already installed.
)

echo  Checking repo package...
"%VENV_PY%" -m pip show usd-convert-cad >nul 2>&1
if errorlevel 1 (
    echo  Installing repo package in editable mode...
    "%VENV_PY%" -m pip install -e "%REPO_ROOT%" --no-deps
    if errorlevel 1 exit /b 1
) else (
    echo  [OK] usd-convert-cad package already installed.
)
...
echo  Checking converter extensions from the Kit registry...
